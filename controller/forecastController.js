...
const GetForecast = async (req, res) => {
  try {
    ...
    for (let i = 0; i < result.data.items[0].forecasts.length; i++) {
      const locationForecast = result.data.items[0].forecasts[i];
      const latLon = result.data.area_metadata[i];
      response.push({ ...latLon.label_location, ...locationForecast });
    }
